...
def calculate_team_average_margins(y, indicators, train_end, z_vec):
    ...
    return 1 - rss/tss, binary_correct/test_games, z_margin, z_games
...
margin_baseline, margin_baseline_binary, z_marg, z_games = calculate_team_average_margins(y=season_margin_y, indicators=season_indicators, train_end=train_end, z_vec=z)
...
baseline_rating_df = generate_team_rating_df(latent_team_dictionary, z_marg, show=True)
...
test_accuracy, test_errors = m.calculate_test_accuracy(season_z, season_lm, season_indicators[train_end:test_end, :], season_x.iloc[train_end:test_end, :], season_margin_y[train_end:test_end])
